...
class Graph( object ):
  ...
  def add_step( s, step ):
    ...
    # Set the order
    assert s.n_steps == len(s._steps) - 1

    step.order = s.n_steps
    s.n_steps += 1
...
'''
  ...
  def topological_sort( s, seed_steps=False ):
    ...
    while( steps ):
      ...
      # + Convert keys to steps
      _steps_without_deps = [ s.get_step( x ) for x in steps_without_deps ]
      ordered_steps_without_deps = sorted( _steps_without_deps )
      steps_without_deps = [ x.get_name() for x in ordered_steps_without_deps ]
