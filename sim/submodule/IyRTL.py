...
class IyVRTL( Placeholder, Component ):
  ...
  def construct( s ,side):
    ...
    # Interface
    s.image_in =[InPort(mk_bits(32))for _ in range((side+2)*(side+2))]
    s.image_out=[OutPort(mk_bits(32))for _ in range(side*side)]
    ...
    s.config_placeholder = VerilogPlaceholderConfigs(
      ...
      src_file = path.dirname(__file__) + '/IyVRTL.v',
