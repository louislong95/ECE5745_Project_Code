...
#if VLINETRACE
#include "obj_dir_Ix/VIx__Syms.h"
#include "svdpi.h"
#endif
...
VIx_t * create_model( const char *vcd_filename ) {
  ...
  for ( int i_0 = 0; i_0 < 15; i_0++ )
    ...
    m->image_in[i_0] = &model->image_in[i_0];

  for ( int i_0 = 0; i_0 < 9; i_0++ )

    m->image_out[i_0] = &model->image_out[i_0];
...
void destroy_model( VIx_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VIx * model = (VIx *) m->model;
...
void trace( VIx_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_Ix__v );
