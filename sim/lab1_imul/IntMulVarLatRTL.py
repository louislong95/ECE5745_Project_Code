...
class IntMulVarLatVRTL( Placeholder, Component ):
  ...
  def construct( s ):
    ...
    s.config_placeholder = VerilogPlaceholderConfigs(
      ...
      src_file = path.dirname(__file__) + '/IntMulVarLatVRTL.v',
      # Name of the Verilog top level module
      top_module = 'lab1_imul_IntMulVarLatVRTL',
      # Port name map
      port_map = {
...
if rtl_language == 'pymtl':
  from .IntMulVarLatPRTL import IntMulVarLatPRTL as _cls
elif rtl_language == 'verilog':
  _cls = IntMulVarLatVRTL
else:
  raise Exception("Invalid RTL language!")

class IntMulVarLatRTL( _cls ):
  def construct( s ):
    super().construct()
    # The translated Verilog must be xRTL.v instead of xPRTL.v
    s.config_verilog_translate = TranslationConfigs(
      translate=False,
      explicit_module_name = f'lab1_imul_IntMulVarLatRTL',
    )
